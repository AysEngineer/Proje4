 ...
 * bir RFID kartıyla iletişimin başlatılmasından, komutların ve verilerin gönderilmesinden ve karttan gelen yanıtın yönetilmesinden sorumludur
 */
 ...
 * RFID okuyucunun (MFRC522 modülü) yakınında mevcut olabilecek birden fazla kartın arasından belirli bir RFID  kartını seçmek için kullanılır
 */
 ...
 * Check: MI_OK if successful
 * kimlik doğrulama başarılı olursa güvenli bloğa erişime izin verir
 */
uint8_t MFRC522_Auth(uint8_t authMode, uint8_t BlockAddr, uint8_t *Sectorkey, uint8_t *serNum)
{
    ...
    buff[0] = authMode;  // 0x60: İki parametre içerir. İlk dört bit, authMode'u (kimlik doğrulama modu) belirtir ve bu değer genellikle 0x60 olarak atanır. Diğer dört bit, blok adresini ve bir bit "Read/Write" belirten "Proximity Integrated Circuit Card (PICC) Command & Parameter" byte'ını belirtir.
    buff[1] = BlockAddr; //3:  RFID kartındaki hangi bloğa erişim izni almak istediğinizi belirtir
    for (i=0; i<6; i++)
    {
		buff[i+2] = *(Sectorkey+i);    // Sectorkey-> Key: Bloğa erişim izni almak için kullanılacak anahtardır. Bu, genellikle bir şifre veya paroladır.
	}
    for (i=0; i<4; i++)
    {
		buff[i+8] = *(serNum+i);
